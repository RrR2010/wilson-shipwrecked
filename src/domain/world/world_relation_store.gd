...
## `_relations_by_key` is truth; other dictionaries are reconstructible indexes.
...
func add_relation(relation: WorldRelation) -> MutationResult:
	...
	if _relations_by_key.has(relation_key):
		return MutationResult.failure(&"duplicate_relation", ["Relation already exists: %s" % relation.sort_key()])
...
func remove_relation(relation: WorldRelation) -> MutationResult:
	...
	if not _relations_by_key.has(relation_key):
		return MutationResult.failure(&"relation_not_found", ["Relation does not exist: %s" % relation.sort_key()])
...
func traverse_relations(start: RuntimeWorldRef, allowed_relation_types: Array, max_depth: int, result_limit: int, direction: int = Direction.OUTGOING) -> RelationTraversalResult:
	...
	while not queued.is_empty():
		...
		if depth >= max_depth:
			continue
		var edges: Array = get_outgoing(current) if direction == Direction.OUTGOING else get_incoming(current)
		for relation in edges:
			if not allowed.is_empty() and not allowed.has(relation.relation_type.key()):
				continue
			if not seen_relations.has(relation.key()):
				seen_relations[relation.key()] = true
				traversed_relations.append(relation)
			var neighbor: RuntimeWorldRef = relation.object if direction == Direction.OUTGOING else relation.subject
			if visited.has(neighbor.key()):
				continue
			visited[neighbor.key()] = true
			subjects.append(neighbor)
			if subjects.size() >= result_limit:
				return RelationTraversalResult.new(subjects, traversed_relations, true)
			queued.append([neighbor, depth + 1])
	return RelationTraversalResult.new(subjects, traversed_relations, false)
...
func validate_indexes() -> MutationResult:
	...
	var expected := {"outgoing": _normalized_index(expected_outgoing), "incoming": _normalized_index(expected_incoming), "by_type": _normalized_index(expected_by_type)}
	var actual := {"outgoing": _normalized_index(_outgoing), "incoming": _normalized_index(_incoming), "by_type": _normalized_index(_by_type)}
	if expected != actual:
...
func _normalized_index(index: Dictionary) -> Dictionary:
	...
	for index_key in index.keys():
		...
		normalized[String(index_key)] = strings
	return normalized
