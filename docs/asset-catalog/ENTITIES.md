...
This is the operational model/content backlog for physical entity families. See [`README.md`](README.md) for ownership, status and column semantics.
...
| TODO | P0 | `pot` | EntityDefinition | medium mass; heat resistance | container; cooking participant | carry when empty/light | empty/full; cold/hot; clean/dirty | inside(food, pot); attached/on support | cooking | Ref 03/11 |
...
A row identifies a semantic/content family, not necessarily one GLB. Do not mark a family `APPROVED` merely because one variant exists if required functional or visual contrasts are still missing.