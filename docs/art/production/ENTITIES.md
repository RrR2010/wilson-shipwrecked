...
This is the operational backlog for physical entity families. See [`README.md`](README.md) for status and column semantics.
...
A row identifies a semantic/art family, not necessarily one GLB. Do not mark a family `APPROVED` merely because one variant exists if required regression contrasts are still missing.