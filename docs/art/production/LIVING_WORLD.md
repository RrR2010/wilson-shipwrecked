...
This is the operational backlog for terrain/place presentation, flora, fauna, habitats and environmental opportunity families. See [`README.md`](README.md) for status and column semantics.
...
A living-world family is not `APPROVED` until it reads at gameplay distance without texture dependence, state changes use broad form/color shifts, ecological variants avoid unnecessary unique meshes, recurring actors stay recognizable without Wilson-level complexity, art sockets remain distinct from domain semantics, and terrain presentation does not become a duplicate gameplay-state model.