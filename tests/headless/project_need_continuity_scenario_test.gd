...
class ActionExecutionStub:
	...
	func advance(execution_id: StringName, _elapsed: float):
		# Each selected fixture action is a one-step completed action. The semantic
		# pressure under test is which intention wins next, not action-duration policy.
...
class SelectedIntentionExecutorStub:
	...
	func _init(
		...
		project_target = p_project_target
		food_target = p_food_target
	...
	func apply(current_intention):
		...
		if current_intention.intention_id.equals(project_intention):
			...
			bindings.bind(&"target", project_target)
			...
			bindings.bind(&"target", food_target)
...
func _run_slice() -> void:
	...
	# One initial checkpoint starts the sequence. After this, action completion and
	# drive-band transitions must carry reconsideration without fixture-authored triggers.
	...
	orchestrator.advance(SimulationStepContext.new(&"project_need_work_1", 1.0, 2.0, null, []))
	...
	_expect_trace_trigger(traces, 1, ReconsiderationGate.Trigger.ACTION_OR_INTENTION_COMPLETION, "first project completion derives reconsideration")
	...
	_expect_current(intentions, project_intention, "food completion autonomously returns Wilson to project")
	_expect_trace_trigger(traces, 3, ReconsiderationGate.Trigger.ACTION_OR_INTENTION_COMPLETION, "food completion itself opens return decision")
...
func _expect_trace_trigger(trace_sink, trace_index: int, trigger: int, label: String) -> void:
