...
func _run_slice() -> void:
	...
	_expect_true(content.register_entity_definition(EntityDefinition.new(
		crate_type, [], {structural_integrity.key(): 5, hardness.key(): 4}, []
	...
	var query_before = DefaultWorldQuery.new(entities, relations, content)
	var profile_before = EffectivePhysicalProfileResolver.new(query_before, graph_before, policies).resolve(crate)
	var resistance_before = profile_before.get_property(effective_resistance)
	...
	var query_after = DefaultWorldQuery.new(restored.entities, restored.relations, content)
	...
	var profile_after = resolver_after.resolve(restored_crate)
	_expect_equal(profile_after.get_property(effective_resistance), resistance_before, "derived physical query matches after rebuild")
