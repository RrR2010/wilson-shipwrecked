...
func _run_relation_store_slice() -> void:
	...
	_expect_true(content.register_entity_definition(EntityDefinition.new(stone_type, [stone_category], {hardness.key(): 3}, [impact_capability])).ok, "stone definition registers")
	_expect_true(content.register_entity_definition(EntityDefinition.new(pouch_type, [], {}, [container_capability])).ok, "pouch definition registers")
	_expect_true(content.register_entity_definition(EntityDefinition.new(crate_type, [], {}, [container_capability])).ok, "crate definition registers")
	...
	var traversal = query.traverse_relations(stone, [inside], 2, 10, WorldRelationStore.Direction.OUTGOING)
