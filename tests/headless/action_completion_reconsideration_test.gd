...
class ActionExecutionStub:
	...
	func advance(execution_id: StringName, _elapsed: float):
		# Completion may occur after an earlier commit tail, so no new outcome is required.
...
func _run_slice() -> void:
	...
	var result = orchestrator.advance(SimulationStepContext.new(
		&"completed_action_without_external_trigger",
		0.1,
		0.1,
		null,
		[]
	...
	_expect_true(result.action_progress != null and result.action_progress.completed, "fixture reaches a real completed ActionProgressResult")
	_expect_equal(result.candidates.size(), 1, "completed action opens reconsideration without an external trigger")
	_expect_true(result.decision != null and result.decision.has_selection(), "completion-derived reconsideration routes a candidate")
	_expect_true(intentions.has_current(), "completion-derived selection commits through the intention owner")
	if intentions.has_current():
		_expect_equal(intentions.current().intention_id.key(), next_intention.key(), "next intention comes from ordinary candidate competition")
	_expect_equal(trace_sink.traces.size(), 1, "completion-derived reconsideration remains traceable")
	if trace_sink.traces.size() == 1:
		var triggers: Array = trace_sink.traces[0].stage_results[&"reconsideration_triggers"]
		_expect_true(triggers.has(ReconsiderationGate.Trigger.ACTION_OR_INTENTION_COMPLETION), "trace exposes internally derived completion trigger")
