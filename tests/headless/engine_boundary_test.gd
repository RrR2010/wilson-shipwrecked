...
func _test_godot_scene_mapping_is_explicit_and_headless_safe() -> void:
	...
	_expect_true(registry.resolve(wilson_ref) == wilson, "semantic ref resolves exact bound Node3D")
	_expect_true(registry.resolve_anchor(coconut_ref, &"pickup") == pickup_anchor, "semantic interaction anchor resolves exact bound Node3D")
	var spatial: Variant = GodotSpatialQueryAdapter.new(registry)
	_expect_true(is_inf(spatial.metric_distance(unknown_ref, coconut_ref)), "unmapped Godot ref fails explicitly before geometry query")
	_expect_true(not spatial.has_route(unknown_ref, coconut_ref), "route query fails closed without mapping/navigation map")
	_expect_true(not spatial.has_line_of_sight(unknown_ref, coconut_ref), "visibility query fails closed without physics space state")
	wilson.free()
	coconut.free()
	pickup_anchor.free()
...
func _test_physical_observations_drain_in_engine_order() -> void:
	var wilson_ref: RuntimeWorldRef = RuntimeWorldRef.wilson()
	var rock_ref: RuntimeWorldRef = RuntimeWorldRef.entity(DomainId.entity(&"falling_rock"))
	var buffer: Variant = GodotPhysicalObservationBuffer.new()
